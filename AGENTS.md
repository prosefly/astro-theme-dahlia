...
Guidance for coding agents working on Astro Theme Dahlia.

## Project Scope

Dahlia is an installable Astro documentation theme. It should behave like an
Astro integration, not like a one-off website template.

The package lives in `packages/astro-theme-dahlia`. The root `src/` directory is
...
logic into Dahlia unless the change is specifically theme-only. Shared MDX
components should be changed in the astro-components package instead.

## Code Organization

- Theme config types live in `packages/astro-theme-dahlia/src/lib/theme.ts`.
- Config defaults, option parsing, and related helpers live in
  `packages/astro-theme-dahlia/src/lib/config/`.
- Sidebar generation and diagnostics live in
  `packages/astro-theme-dahlia/src/lib/sidebar/`.
- Page metadata, edit links, schema, last-updated, and page actions helpers live
  in `packages/astro-theme-dahlia/src/lib/page/`.
- Search provider logic lives in `packages/astro-theme-dahlia/src/lib/search/`
  and `packages/astro-theme-dahlia/src/client/search/`.
- Markdown transforms live in `packages/astro-theme-dahlia/src/lib/markdown/`.
- Layout shell components live in
  `packages/astro-theme-dahlia/src/components/layout/`.
- Public reusable theme components live in
  `packages/astro-theme-dahlia/src/components/theme/`.
- Override defaults live in
  `packages/astro-theme-dahlia/src/components/defaults/`.
...
Use Dahlia CSS tokens instead of hard-coded colors or spacing when possible.
