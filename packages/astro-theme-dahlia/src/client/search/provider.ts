...
export function createSearchProvider(dialog: Element): SearchProvider | undefined {
  const provider = dialog.getAttribute('data-dahlia-search-provider') || 'local';

  if (provider === 'local') {
    const indexUrl = dialog.getAttribute('data-dahlia-search-index');

    return indexUrl ? createLocalSearchProvider(indexUrl) : undefined;
  }

  if (provider === 'pagefind') {
    const bundlePath = dialog.getAttribute('data-dahlia-pagefind-bundle');
