...
export function createSearchMessageElement(message: string): HTMLElement {
  ...
  empty.className = 'px-3 py-8 text-center text-sm text-(--dahlia-text-muted)';
...
export function createSearchResultElement(
  ...
  meta.className = 'mb-1 flex min-w-0 items-center gap-2 text-xs text-(--dahlia-text-muted)';
  ...
  title.className = 'truncate text-sm font-medium text-(--dahlia-text-strong)';
  title.textContent = item.title ?? '';

  const excerpt = document.createElement('p');
  excerpt.className = 'mt-1 line-clamp-2 text-sm leading-6 text-(--dahlia-text-muted)';
