...
function initThemeSwitches(): void {
  const switches = Array.from(document.querySelectorAll<HTMLElement>('[data-dahlia-theme-switch]'));
  ...
  switches.forEach((switcher) => {
    if (switcher.dataset.dahliaThemeSwitchReady) {
      return;
    }

    switcher.dataset.dahliaThemeSwitchReady = 'true';
