...
export function getLocales(config: DahliaThemeConfig): NormalizedLocale[] {
...
export function getDefaultLocale(config: DahliaThemeConfig): NormalizedLocale {
...
export function getLocalizedSearchHref(config: DahliaThemeConfig, localeKey?: string): string {
  const locale = getLocaleByKey(config, localeKey);
  const basePath = config.docsBase === '/' ? '' : config.docsBase.replace(/\/$/, '');
