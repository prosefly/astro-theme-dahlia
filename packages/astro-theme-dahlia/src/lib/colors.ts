...
export function isPresetAccent(accent: string): accent is AccentScale {
  return presetAccents.has(accent);
}
...
export function getCustomAccentVariables(accent: string): Record<string, string> {
  const color = hexToOklch(accent);

  if (!color) {
    return {
      '--dahlia-accent-light': accent,
      '--dahlia-accent-dark': accent,
    };
  }
  ...
  return {
    '--dahlia-accent-light': formatOklch(0.58, chroma, color.hue),
    '--dahlia-accent-dark': formatOklch(0.72, darkChroma, color.hue),
  };
}
