  ...
  it('supports custom heading ids and removes the marker text', () => {
    const tree = runHeadingIds({
      ...
      children: [
        heading([
          text('Install Dahlia '),
          { type: 'idString', value: 'install-dahlia' },
        ]),
      ],
    });

    expect(tree.children?.[0].data?.hProperties?.id).toBe('install-dahlia');
    expect(tree.children?.[0].children).toEqual([text('Install Dahlia')]);
  });
