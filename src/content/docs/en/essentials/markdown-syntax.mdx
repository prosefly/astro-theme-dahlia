...
description: Reference the core Markdown patterns supported inside Dahlia docs.

---

Dahlia uses Astro content collections and MDX, so standard Markdown remains the
...
Dahlia removes the marker from the rendered heading, uses `manual-setup` for the
heading `id`, and keeps table-of-contents links in sync.

Do not add a top-level `# Heading` at the start of the MDX body. Dahlia renders
...
```md
![Dahlia screenshot](/images/dahlia-screenshot.png)
```
...
Unordered lists use `-`, `*`, or `+`. Dahlia examples prefer `-`.
...
```md
> Dahlia is designed for documentation that should feel compact, readable, and
> easy to scan.
```
...
```md
Run `pnpm dev` and edit `theme.config.json`.
```
...
Dahlia uses Expressive Code for fenced code blocks by default. See
