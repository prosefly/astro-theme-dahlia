...
Dahlia includes a provider-based search dialog. The default provider is `local`,
...
Dahlia supports the built-in local provider, Pagefind, and Algolia DocSearch.
...
Pagefind is an optional peer dependency. Install it before selecting this
provider:
...
When enabled, Dahlia runs Pagefind after `astro build`, indexes the generated
...
advanced Pagefind setups, but most Dahlia sites should rely on the built-in
...
The default Dahlia search trigger opens the DocSearch modal through the
DocSearch JavaScript API.

For localized DocSearch indexes, `indexName` can be a map keyed by Dahlia locale
...
DocSearch crawling and indexing are outside Dahlia. Use a search-only API key in
...
The field name is kept for Starlight compatibility. In Dahlia it controls the
built-in `search.json` output. When the active provider is Pagefind, Dahlia also
...
search it emits `data-search-provider` and `data-search-index`.
For Pagefind search it also emits `data-pagefind-bundle`, and the page
...
Dahlia search is intentionally lightweight. It does not provide stemming,
