...
Dahlia can load hosted Ask AI widgets for documentation sites. The theme does
...
When an assistant provider is configured, Dahlia adds an AI trigger button next
...
Dahlia loads the Inkeep browser script only when this provider is enabled and
...
with Dahlia' `data-theme` attribute so the widget follows light and dark mode.
...
assistant: {
  ...
  aiChatSettings: {
    ...
    exampleQuestions: [
      'How do I configure docs navigation?',
...
Dahlia renders Kapa's widget script only when this provider is enabled. The
...
The default custom provider renders nothing. This keeps Dahlia out of API keys,
