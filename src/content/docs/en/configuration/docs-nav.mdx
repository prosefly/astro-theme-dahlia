...
You can also pass the same structure to `dahlia({...})` in `astro.config.ts`:
...
`slug` is optional. If omitted, Dahlia derives it from the label. Add a slug when
...
String items use the page's frontmatter `title` as their label and its optional
`sidebar.icon` as their icon. A localized page can override the default locale's
icon; otherwise it inherits that icon automatically.
...
{
  label: 'GitHub',
  link: 'https://github.com/prosefly/astro-theme-dahlia',
  external: true,
...
Use `translations` for labels owned by `theme.config.json`. This includes
...
The `directory` value follows the same locale-neutral rule. For example,
...
By default, Dahlia renders autogenerated entries as a flat list and includes all
