---
title: Integrations
description: Configure Iconify, Expressive Code, Mermaid, callout directives, CJK Markdown, and package manager tabs.

---

import { Card, CardGrid } from '@prosefly/astro-components';

Dahlia includes integrations for icons, syntax highlighting, Mermaid diagrams,
callout directives, CJK-friendly Markdown, image galleries, and package-manager
command tabs.
...
<CardGrid>
  ...
  <Card icon="lucide:workflow" title="Mermaid" href="#mermaid">
    Render fenced Mermaid diagrams as theme-aware static SVG during the build.
  </Card>
</CardGrid>

## Iconify

Dahlia installs the `@prosefly/astro-components/integration` integration internally.
...
Mermaid rendering is enabled by default through the shared Astro Components
integration. Write a fenced `mermaid` code block to generate a static SVG during
content processing; Dahlia also excludes Mermaid source from Shiki syntax
highlighting.

See the [Mermaid reference](/docs/references/mermaid/) for supported diagram
types, syntax examples, rendering behavior, accessibility, styling, and
troubleshooting.

Disable the transform when Mermaid source should remain a normal code block.
