...
Dahlia follows Starlight's i18n model: each locale has a key, a label, a
...
Dahlia sets the page `<html dir>` from each locale. If `dir` is omitted, common
...
`locales.*.directory` strips the locale directory before Dahlia matches sidebar
...
`configuration/project` in `docsNav`.
...
If a non-default locale does not include a page, Dahlia can fall back to the
...
Dahlia includes template translations for common documentation site locales.
...
Labels written in `theme.config.json` are project content, not built-in UI
...
The same pattern is supported by site nav items, social links, docs nav sections,
sidebar groups, and sidebar link items. Page links generated from content use
the localized page frontmatter instead.

## Translator API

Dahlia provides the resolved translator as `Astro.locals.t` for theme components
...
current locale's text direction, and `t.locale()` to read the resolved Dahlia
locale object.
