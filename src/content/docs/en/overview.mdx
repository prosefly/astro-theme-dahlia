---
title: Overview
description: Learn what Dahlia provides and how the documentation theme is organized.
sidebar:
  icon: "lucide:map"
...
<CardGrid>
  ...
  <Card icon="lucide:blocks" title="MDX components" href="/docs/components/callout/">
    ...
    badges, media, embeds, and Iconify icons.
  </Card>
...
## The four parts of a Dahlia site

1. `astro.config.ts` registers the Dahlia integration.
2. `src/content.config.ts` defines a `docs` collection with Dahlia's loader and
   schema.
3. `theme.config.json` controls identity, routes, navigation, search, source
   links, appearance, and component overrides.
4. `src/content/docs/**/*.mdx` contains the documentation pages.

Dahlia reads the collection and injects page, Markdown source, and search routes
under `docsBase`. With the default `docsBase: '/'`, it generates:
