---
title: 安装
description: 从 starter template 创建 Dahlia 文档站点，或将 Dahlia 加到现有 Astro 项目。
...
站点跑起来后，替换示例内容，并在 Dahlia theme config 中更新项目名称、导航、
...
3. **创建 theme config。**
   ...
   里传入 `dahlia({...})`。这些选项会覆盖 `theme.config.json` 中的值。

4. **注册 docs collection。**

   创建 `src/content.config.ts`，并使用 Dahlia 提供的 loader 和 schema。
...
5. **添加第一篇文档。**
   ...
   Dahlia 默认从 `src/content/docs` 读取文档。
