...
Dahlia 采用类似 Starlight 的 i18n 模型：每个 locale 都有 key、label、language tag、
...
Dahlia 会根据每个 locale 设置页面 `<html dir>`。省略 `dir` 时，会根据 `lang` 或 locale
key 推断常见 RTL 语言，例如 `ar`、`fa`、`he` 和 `ur`。需要覆盖默认推断时，显式设置 `dir`。

## Directory Mapping

`locales.*.directory` 会在 Dahlia 匹配 sidebar string items 和
`autogenerate.directory` values 前移除 locale 目录。

例如，`src/content/docs/en/configuration/project.mdx` 和
`src/content/docs/zh-cn/configuration/project.mdx` 在 `docsNav` 中都引用为
...
如果非默认 locale 没有某个页面，Dahlia 可以回退到默认 locale entry。这样在翻译进行中，
部分翻译站点仍然可以正常导航。
...
Dahlia 为常见文档站点 locale 内置 template translations。这些字符串覆盖搜索、theme mode
...
写在 `theme.config.json` 中的 labels 属于项目内容，不属于内置 UI messages。当同一个配置项需要在不同
...
site nav items、social links、docs nav sections、sidebar groups 和 sidebar link items
都支持同样模式。从内容生成的页面链接会使用本地化页面 frontmatter。

## Translator API

Dahlia 将解析后的 translator 作为 `Astro.locals.t` 提供给主题组件和本地 overrides。
...
`t.locale()` 读取解析后的 Dahlia locale object。
