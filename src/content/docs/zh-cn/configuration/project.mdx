...
`name` 是公开的产品名或项目名。Dahlia 会将它用于站点标题、header brand text、
...
Dahlia 会将这个值用于默认 `<meta name="description">` 和 structured data。

## Logo

`logo` 是可选的。省略时，Dahlia 会渲染内置 Dahlia mark。

默认情况下，Dahlia 会把 logo 当作 mark，并把它和 `name` 一起渲染。当同一个 mark
...
如果 logo asset 已经包含项目名，设置 `variant: 'lockup'`。Dahlia 会只渲染图片，
...
本地 assets，Dahlia 会在 Astro config setup 阶段读取图片尺寸，并自动渲染 `width`
和 `height` attributes。只有远程 logo URL 或 Dahlia 无法从 `public/` 读取的路径，
