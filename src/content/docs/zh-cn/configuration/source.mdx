...
`source` 描述文档文件在版本控制中的位置。Dahlia 会用它把渲染后的页面映射回源文件，
...
Dahlia 从 Astro content entry metadata 读取源文件路径，所以这里只需要提供仓库信息。
Repository 字段既可以使用 shorthand，也可以使用完整 repository URL。

## Repository Providers

省略 `source` 时，Dahlia 会尝试从 `git config --get remote.origin.url` 推断 repository
provider 和 URL。这支持常见 GitHub、GitLab 和 Codeberg remotes，也支持
`git@github.com:owner/repo.git` 这样的 SSH remote。

Dahlia 不会从 git 推断 branch。链接不指向 `main` 时，需要设置 `source.branch`。
...
Dahlia 使用每个 entry 的真实 `filePath`，为配置的 repository provider 构建 edit URL。
你不需要在 source 配置中重复 `docsLoader({ base })` 目录。

对于 `src/content/docs/en/overview.mdx`，Dahlia 会生成：
...
Dahlia 可以在 previous/next links 上方显示页面 contributors。

```ts
contributors: true
```

Contributors 使用与 edit links 相同的 entry file path。Dahlia 会先从本地 `git log`
...
Dahlia 还会为前 10 个未排除的 repository contributors 加载 GitHub user profiles，以便用
...
使用 `githubProfileLimit` 修改 Dahlia 请求多少个 GitHub user profiles 来做 full-name matching。
...
Dahlia 会自动使用 `GITHUB_TOKEN` 或 `GH_TOKEN` 环境变量。GitHub request 失败时，
Dahlia 会回退到本地 git history，因此静态构建仍然可以完成。
...
`/docs/overview/`，Dahlia 使用的 source path 仍然是
